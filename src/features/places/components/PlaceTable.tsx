import { useEffect, useMemo, useState } from 'react'
import type { Place } from '@prisma/client'
import { createPlaceHash } from '@/lib/hash'
import type { Coordinate } from '@/lib/geo'

interface PlaceTableProps {
    places: Place[]
    onPlaceSelect: (place: Place) => void
    onRouteSelectionChange: (selectedPlaces: Coordinate[]) => void
    onDeletePlace?: (placeId: string) => Promise<void> | void
    deletingPlaceId?: string | null
}

export function PlaceTable({
    places,
    onPlaceSelect,
    onRouteSelectionChange,
    onDeletePlace,
    deletingPlaceId = null,
}: PlaceTableProps) {
    const [searchTerm, setSearchTerm] = useState('')
    const [selectedPlaceIds, setSelectedPlaceIds] = useState<string[]>([])

    useEffect(() => {
        setSelectedPlaceIds((currentSelectedIds) =>
            currentSelectedIds.filter((placeId) => places.some((place) => place.id === placeId))
        )
    }, [places])

    const filteredPlaces = useMemo(() => {
        const normalizedSearchTerm = searchTerm.trim().toLowerCase()

        if (!normalizedSearchTerm) {
            return places
        }

        return places.filter((place) => place.name.toLowerCase().includes(normalizedSearchTerm))
    }, [places, searchTerm])

    useEffect(() => {
        onRouteSelectionChange(
            places
                .filter((place) => selectedPlaceIds.includes(place.id))
                .map((place) => ({
                    latitude: place.latitude,
                    longitude: place.longitude,
                }))
        )
    }, [onRouteSelectionChange, places, selectedPlaceIds])

    function toggleRouteSelection(place: Place) {
        setSelectedPlaceIds((currentSelectedIds) => {
            const isAlreadySelected = currentSelectedIds.includes(place.id)

            if (isAlreadySelected) {
                return currentSelectedIds.filter((placeId) => placeId !== place.id)
            }

            if (currentSelectedIds.length >= 2) {
                return currentSelectedIds
            }

            return [...currentSelectedIds, place.id]
        })
    }

    const canSelectRoutingPoints = places.length > 1

    return (
        <div className="flex flex-col">
            <div className="overflow-x-auto ml-3 mr-4">
                <div className="min-w-full inline-block align-middle">
                    <div className="rounded-lg border divide-y divide-gray-200">
                        <div className="py-3 px-4">
                            <div className="relative max-w-xs">
                                <label htmlFor="search-place" className="sr-only">
                                    Search Place
                                </label>
                                <input
                                    type="text"
                                    name="search-place"
                                    id="search-place"
                                    className="p-3 pl-10 block w-full rounded-md border border-gray-200 text-sm text-gray-800 outline-none ring-gray-300 transition duration-100 focus-visible:ring"
                                    placeholder="Search Place"
                                    value={searchTerm}
                                    onChange={(event) => setSearchTerm(event.target.value)}
                                    autoComplete="off"
                                />
                                <div className="absolute inset-y-0 left-0 flex items-center pl-4">
                                    <svg
                                        className="h-3.5 w-3.5 text-black"
                                        xmlns="http://www.w3.org/2000/svg"
                                        width="16"
                                        height="16"
                                        fill="currentColor"
                                        viewBox="0 0 16 16"
                                    >
                                        <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z" />
                                    </svg>
                                </div>
                            </div>
                        </div>
                        <div className="overflow-hidden rounded-b-lg">
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        <th
                                            scope="col"
                                            className="pl-2 py-3 text-left text-xs font-medium text-gray-500 uppercase"
                                        >
                                            Routing
                                            <br />
                                            Point
                                        </th>
                                        <th
                                            scope="col"
                                            className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase"
                                        >
                                            Place Name
                                        </th>
                                        <th
                                            scope="col"
                                            className="pr-3 py-3 text-right text-xs font-medium text-gray-500 uppercase"
                                        />
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-200">
                                    {filteredPlaces.length === 0 ? (
                                        <tr>
                                            <td
                                                colSpan={3}
                                                className="px-3 py-8 text-center text-sm text-gray-500"
                                            >
                                                No places found.
                                            </td>
                                        </tr>
                                    ) : (
                                        filteredPlaces.map((place, index) => {
                                            const isSelected = selectedPlaceIds.includes(place.id)
                                            const disableUncheckedRouting =
                                                !isSelected && selectedPlaceIds.length >= 2

                                            return (
                                                <tr key={place.id}>
                                                    <td className="py-3 pl-4">
                                                        <div className="flex items-center h-5">
                                                            {canSelectRoutingPoints ? (
                                                                <>
                                                                    <input
                                                                        id={`routing-point-${place.id}`}
                                                                        type="checkbox"
                                                                        className="rounded border-gray-200 text-gray-600 accent-gray-800 focus:ring-gray-500"
                                                                        checked={isSelected}
                                                                        disabled={
                                                                            disableUncheckedRouting
                                                                        }
                                                                        onChange={() =>
                                                                            toggleRouteSelection(
                                                                                place
                                                                            )
                                                                        }
                                                                    />
                                                                    <label
                                                                        htmlFor={`routing-point-${place.id}`}
                                                                        className="sr-only"
                                                                    >
                                                                        Routing Point {index + 1}
                                                                    </label>
                                                                </>
                                                            ) : null}
                                                        </div>
                                                    </td>
                                                    <td className="px-3 py-4 whitespace-nowrap text-sm font-medium text-gray-800">
                                                        <a
                                                            href={`#${createPlaceHash(place.name)}`}
                                                            onClick={(event) => {
                                                                event.preventDefault()
                                                                onPlaceSelect(place)
                                                            }}
                                                        >
                                                            {place.name}
                                                        </a>
                                                    </td>
                                                    <td className="pr-3 py-3 items-center">
                                                        {onDeletePlace ? (
                                                            <button
                                                                type="button"
                                                                className="text-gray-800 hover:text-gray-500 disabled:cursor-not-allowed disabled:text-gray-300"
                                                                disabled={
                                                                    deletingPlaceId === place.id
                                                                }
                                                                onClick={() =>
                                                                    onDeletePlace(place.id)
                                                                }
                                                            >
                                                                <span className="sr-only">
                                                                    Delete {place.name}
                                                                </span>
                                                                <svg
                                                                    xmlns="http://www.w3.org/2000/svg"
                                                                    fill="none"
                                                                    viewBox="0 0 24 24"
                                                                    strokeWidth={1.5}
                                                                    stroke="currentColor"
                                                                    className="w-5 h-5"
                                                                >
                                                                    <path
                                                                        strokeLinecap="round"
                                                                        strokeLinejoin="round"
                                                                        d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
                                                                    />
                                                                </svg>
                                                            </button>
                                                        ) : null}
                                                    </td>
                                                </tr>
                                            )
                                        })
                                    )}
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

